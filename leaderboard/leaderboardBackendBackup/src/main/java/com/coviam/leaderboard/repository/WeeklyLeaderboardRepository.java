...
public interface WeeklyLeaderboardRepository extends CrudRepository<WeeklyLeaderboard,WeeklyLeaderboardPK> {
    ...
    @Query(value = "select * from weekly_leaderboard where week_id = ?1 order by score desc", nativeQuery = true)
    List<WeeklyLeaderboard> findAllByOrderByUserScoreDesc(long weekId);
}
